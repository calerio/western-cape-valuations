...
1. **Owner names are in the public data (POPIA).** In Matzikama, the 864 rows with `suburb='0'` are column-shifted. `site_address` holds the registered owner's name (a person's or a family trust's name) and `category` holds the town name. `site_address` is exported to `search.db` as `prop.address`, so these names are **live**. Test: `SELECT site_address FROM property WHERE municipality_id=12 AND suburb='0'`. A heuristic scan of the other municipalities found only street or estate names, but it was not exhaustive. **Blank `site_address` for these rows before any further export, and never show it on the Explore page.** Fixed 2026-09-23 by the Matzikama parser repair (DATA_CONTRACT §7).
