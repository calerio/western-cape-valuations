...
async function boot() {
  ...
  try {
    ...
    [STATS, TOWNS, PROV, DISTF, MUNIF, EXPLORE] = await Promise.all([
      ...
      fetch("data/geo/za-outline.geojson").then(r => r.json()),
  ...
  idle(() => ensureDB().catch(() => {}), { timeout: 4000 });
}
...
function initMap() {
  ...
  // za-outline.geojson: "South Africa" (whole country) + "Western Cape". The country is filled with the
  // Western Cape cut out (evenodd with the WC ring appended), so the province itself stays unfilled
  // under the district/municipality layers exactly as with the old per-province file.
  const wcF = PROV.features.find(f => name(f) === "Western Cape");
  gProv.selectAll("path").data(PROV.features).join("path")
    .attr("d", d => name(d) === "Western Cape" || !wcF ? path(d) : path(d) + path(wcF))
...
/* South Africa locator (decorative, aria-hidden): the country outline at 120×90 with the Western
 * Cape inked. Reuses the already-loaded za-outline.geojson (two features: "South Africa", the
 * provinces dissolved, then "Western Cape" — generated by the data repo's extract/geo/simplify_geo.py). */
...
async function ensureDB() {
  ...
  if (!dbwPromise) dbwPromise = (async () => {
    ...
    const DB_CONFIG = "https://nxeasppmwvzcqbbgrdvf.supabase.co/storage/v1/object/public/valuations/b-93c01c0b6202/config.json";
