...
- **No fabricated geography.** Every boundary shown comes from an official published source —
  we never invent or approximate one (the old Voronoi "town" level was removed for exactly that
  reason). Municipality remains the finest level on the Atlas **choropleth**; since 2026-07-19 the
  two map views also show real town/suburb boundaries from the Stats SA Census place-name layers
  hosted by the WC Government GIS (`StatsSA_CensusBoundaries` MapServer — the same host as the
  cadastre), live-fetched on search selection (DATA_CONTRACT §14). Atlas suburb names still jump
...
- `map.html` (satellite, pins dark theme) + `plain.html` (OpenFreeMap vector basemap, pins light
  theme) — both driven by the shared `assets/map.js` (`<body data-basemap="plain">` selects the
  mode) with `assets/places.js` for the town/suburb search + boundary highlight
  (DATA_CONTRACT.md §14). `plain.html` is a hand-maintained near-copy of `map.html` — keep their
  shared markup/CSS in sync when editing either. Bump `?v=` on `map.js`/`places.js` tags on change.
...
- `data/` — `stats.json`, `towns.json`, `geo/*.geojson` (incl. the generated `geo/places.json`
  search index — DATA_CONTRACT.md §14), `db/` (chunked SQLite + `config.json`).
